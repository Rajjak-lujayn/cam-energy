...
<div class="yith-ywraq-mail-form-wrapper">
    <?php
    add_action('woocommerce_after_shipping_rate', 'add_custom_shipping_field');

function add_custom_shipping_field($shipping_rate) {
    $shipping_method_id = $shipping_rate->get_method_id();
    
    // Add your condition to display the field only for specific shipping methods if needed
    if ($shipping_method_id === 'flat_rate') {
        // Add a custom text input field
        woocommerce_form_field('custom_shipping_field', array(
            'type'      => 'text',
            'class'     => array('form-row-wide'),
            'label'     => __('Custom Shipping Field', 'woocommerce'),
            'placeholder' => __('Enter your custom field', 'woocommerce'),
            'required'  => true,
        ), WC()->checkout->get_value('custom_shipping_field'));
    }
}

// Validate and save the custom field
add_action('woocommerce_checkout_process', 'validate_custom_shipping_field');

function validate_custom_shipping_field() {
    if (isset($_POST['custom_shipping_field']) && empty($_POST['custom_shipping_field'])) {
        wc_add_notice(__('Please enter a value for the custom shipping field.', 'woocommerce'), 'error');
    }
}

// Save the custom field value to the order meta
add_action('woocommerce_checkout_create_order', 'save_custom_shipping_field');

function save_custom_shipping_field($order) {
    if (isset($_POST['custom_shipping_field'])) {
        $order->update_meta_data('_custom_shipping_field', sanitize_text_field($_POST['custom_shipping_field']));
    }
}?>
